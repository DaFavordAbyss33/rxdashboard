...
async function getGuildInfo(botId: BotId, guildId: string) {
  ...
  try {
    ...
    console.error(`[urgent-dm] Failed to get guild ${guildId}:`, error)
    return null
  }
}
...
async function createDMChannel(botId: BotId, userId: string) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[urgent-dm] Failed to create DM channel: ${response.status}`)
      return null
    }
    return response.json()
  } catch (error) {
    console.error(`[urgent-dm] Failed to create DM channel:`, error)
    return null
  }
}
...
async function sendDM(botId: BotId, channelId: string, content: string) {
  ...
  try {
    const response = await fetch(`${DISCORD_API_BASE}/channels/${channelId}/messages`, {
      ...
      body: JSON.stringify({
        ...
        embeds: [{
          ...
          description: content,
          color: 0xFF6B6B, // Red-ish color for urgency
          timestamp: new Date().toISOString(),
          footer: {
            text: "This is an automated urgent notification",
          },
        }],
      }),
    })
    ...
    if (!response.ok) {
      ...
      console.error(`[urgent-dm] Failed to send DM: ${response.status}`, errorData)
      return false
    }
    return true
  } catch (error) {
    console.error(`[urgent-dm] Failed to send DM:`, error)
    return false
  }
}
...
export async function GET() {
  try {
    ...
    console.error("[urgent-dm] Failed to fetch history:", error)
    return NextResponse.json(
...
// POST - Send urgent DMs to guild owners
  ...
  try {
    ...
    if (targetGuildIds.length === 0) {
      ...
      try {
        ...
        console.error("[urgent-dm] Failed to fetch guilds:", error)
      }
    }
    ...
    for (const guildId of targetGuildIds) {
      try {
        ...
        console.error(`[urgent-dm] Error processing guild ${guildId}:`, error)
        failedGuilds.push(guildId)
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Sent urgent DMs to ${successCount} guild owner(s)`,
      successCount,
      failedCount: failedGuilds.length,
      failedGuilds: failedGuilds.slice(0, 10), // Limit to first 10 for response size
    })
  } catch (error) {
    console.error("[urgent-dm] Failed to send DMs:", error)
    return NextResponse.json(
